...
class ProteinRankNet(nn.Module):
    def __init__(self, input_shape):
        ...
        # 3D卷积层
        self.conv3d_1 = nn.Conv3d(in_channels=1, out_channels=16, kernel_size=(3, 3, 3), padding=1)
        ...
        self.conv3d_bn3 = nn.BatchNorm3d(64)

        # AKConv 2D卷积层
        self.akconv2d_1 = AKConv(64, 128, num_param=3)
        self.akconv2d_bn1 = nn.BatchNorm2d(128)
        
        # 添加更多的2D卷积层
        self.akconv2d_2 = AKConv(128, 256, num_param=3)
        self.akconv2d_bn2 = nn.BatchNorm2d(256)
        
        # Dropout层
        self.dropout = nn.Dropout(p=0.5)

        # LSTM层
        self.lstm = nn.LSTM(input_size=256 * 20, hidden_size=128, num_layers=2, batch_first=True)
    ...
    def forward(self, x):
        ...
        # 将2D卷积层输出转换为LSTM输入
        x = x.view(x.size(0), -1, 256 * 20)  # 调整为 (batch_size, seq_len, features)
