...
# ----------改进编码----------
# 接触序列编码，将接触信息进行叠加
def one_hot_contacts_(seq_input, ss, contacts):
    # make contact directory
    my_contact_dict = {} # len=441
    AAs = ['-','A','C','D','E','F','G','H','I','K','L','M','N','P','Q','R','S',\
        'T','V','W','Y']
    count = 0
    for k in AAs:
    ...
    X = np.zeros((n, len(my_contact_dict)*len(seq_input[0]))) # 441*len(seq)
    ...
    for i, seq in enumerate(seq_input):
        for p, (j, k) in enumerate(contacts):
            # find the contact
            contact_index = my_contact_dict[(seq[j],seq[k])]
            X[i][len(my_contact_dict)*j+contact_index] = 1 # 441*j+contact_index
...
def one_hot_(seq_input, ss, contacts):
    # reshape to contain both contact and sequence info
    X_seq = one_hot_seq(seq_input)
    # X_contact = one_hot_contacts(seq_input, ss, contacts) # 原始接触编码(B,cont*441)
    X_contact = one_hot_contacts_(seq_input, ss, contacts) # 修改接触编码(B,l*441)
